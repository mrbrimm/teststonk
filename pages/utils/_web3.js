...
export const mintWhitelist = async (account, proof) => {
  ...
  const result = sampleNFT.methods.mintWhitelist(proof, 1).send({ from: account, value: amountToWei }).then((result) => {
    console.log(`✅ Check out your transaction on Etherscan: https://rinkeby.etherscan.io/tx/` + result.transactionHash);
    return {
      success: true,
      status: `✅ Check out your transaction on: <a href="https://rinkeby.etherscan.io/tx/` + result.transactionHash + `">Etherscan</a>`
      };
  }).catch((err) => {
  ...
  export const mintPublic = async (account, _mintAmount) => {
    console.log('minting publicMint...');
    const amount = (_mintAmount * 0.01).toString();
    const amountToWei = web3.utils.toWei(amount, 'ether');
    const result = sampleNFT.methods.mint(_mintAmount).send({ from: account, value: amountToWei }).then((result) => {
      console.log(result);
